...
FROM
superstore s1
JOIN superstore s2 ON
s1.Customer_ID = s2.Customer_ID 
AND s2.Order_Date = DATE_ADD(s1.Order_Date, INTERVAL 1 DAY)
JOIN superstore s3 ON s1.Customer_ID = s3.Customer_ID AND s3.Order_Date = DATE_ADD(s2.Order_Date, INTERVAL 1 DAY)
WHERE
s1.Sales > 50 AND
s2.Sales > 50 AND s3.Sales > 50
